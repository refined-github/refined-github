...
function changeTabToTags(): void {
	// Select "Tags" tab
	$('.rgh-tags-dropdown button.SelectMenu-tab:last-child')!.click();
}

function updateLinksToTag(): void {
	// Change links, which point to the content of each tag, to open the tag page instead
	for (const anchorElement of $$('.rgh-tags-dropdown #tags-menu a.SelectMenu-item[href*="/tree/"]')) {
...
function init(): void {
	...
	$('.rgh-tags-dropdown')!.addEventListener('remote-input-success', updateLinksToTag);
}
