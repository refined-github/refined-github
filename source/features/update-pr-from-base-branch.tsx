...
function createButton(): JSX.Element {
	return (
		<button type="button" className="btn btn-sm rgh-update-pr-from-base-branch tooltipped tooltipped-sw" aria-label="Use Refined GitHub to update the PR from the base branch">
			Update branch
		</button>
	);
}
...
async function addButton(mergeBar: Element): Promise<void> {
	...
	if (mergeabilityRow) {
		...
		mergeabilityRow.prepend(<div className="branch-action-btn float-right js-immediate-updates js-needs-timeline-marker-header">{createButton()}</div>);
		return;
	}

	// The PR is still a draft
	mergeBar.before(
		createMergeabilityRow({
