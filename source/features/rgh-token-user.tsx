	...
	if (currentWebUser !== currentTokenUser) {
		header.after(
			...
			<div className="flash px-3 mt-3 mb-0 py-2 d-flex flex-items-center border-0 rounded-0">
				...
				<span>Your <OptionsLink className="btn-link">Rough GitHub token</OptionsLink> is for a different user, the extension will act on behalf of <code>{currentTokenUser}</code></span>
			</div>,
		);
	}
}
