...
const hasAnyReleases = () => select('.release-entry') !== null || select('.release') !== null;
...
const fetchReleasesV3 = async (ownerName, repoName): Promise<Release[]> => {
	return new Promise(async (resolve, reject) => {
		try {
			const jsonResponse = (await api.v3(
				`repos/${ownerName}/${repoName}/releases?per_page=100`
			)) as ReleasesResponseV3[];
