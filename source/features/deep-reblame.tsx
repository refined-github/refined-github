...
const getPullRequestBlameCommit = mem(
	async (
		...
		if (
			!associatedPr ||
			!prNumbers.includes(associatedPr.number) ||
			associatedPr.mergeCommit.oid !== commit
		) {
...
async function redirectToBlameCommit(
	...
	const commitInfo = $('span[data-hovercard-url*="/commit/"]', blameHunk)
		.dataset.hovercardUrl!;
	const prCommit = extractCommitFromHoverCardUrl(commitInfo);
	const blameUrl = new GitHubFileUrl(location.href);

	await showToast(
		async () => {
			blameUrl.branch = await getPullRequestBlameCommit(
				prCommit,
				prNumbers,
				blameUrl.filePath,
			);
...
function addButton(hunk: HTMLElement): void {
	...
	if (reblameLink) {
		reblameLink.setAttribute(
			'aria-label',
			'View blame prior to this change. Hold `Alt` to extract commits from this PR first',
		);
...
async function init(signal: AbortSignal): Promise<void> {
	...
	observe(
		'.react-blame-for-range:has([data-hovercard-type="pull_request"])',
		addButton,
		{signal},
	);
}
