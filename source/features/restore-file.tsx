...
async function getFile(filePath: string): Promise<string | undefined> {
	...
	const { content, httpStatus } = await api.v3(`contents/${filePath}?ref=${ref}`, {
...
function getFilenames(menuItem: HTMLElement): { original: string; new: string } {
	...
	const [originalFileName, newFileName = originalFileName] = (span ?? fileNameElement).textContent // eslint-disable-next-line unicorn/prefer-string-replace-all -- Invisible char
		.split("  ")
...
async function handleClick(event: DelegateEvent<MouseEvent, HTMLButtonElement>): Promise<void> {
	...
	await showToast(
		async (progress) => discardChanges(progress, filenames.original, filenames.new, commitTitle),
		{
