...
function autoOpenModal(signal: AbortSignal): void {
	...
	observe('.js-repo-delete-proceed-confirmation', field => {
		const notice = (
			<p className="mt-2 color-fg-muted">
				Type one character per minute. Your patience will be rewarded with absolutely nothing.
			</p>
		);
		...
		field.addEventListener('beforeinput', event => {
			...
			if (Date.now() - lastInputTime < 60_000) {
...
5. On the last step, the repo name field should let you type only one character per minute
