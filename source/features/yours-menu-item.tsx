...
function init() {
	...
	const yoursMenuItem = <a href={`${location.pathname}?q=is%3Aopen+archived%3Afalse+is%3Aissue+user%3A${username}`} className="subnav-item">Yours</a>;

	if (!select.exists('.subnav-links .selected') && location.search.includes(`user%3A${username}`)) {
		yoursMenuItem.classList.add('selected');
		for (const tab of select.all<HTMLAnchorElement>(`.subnav-links a[href*="user%3A${username}"]`)) {
