...
function addQuickButtons(contextMenuIcon: HTMLElement): void {
	...
	button.replaceChildren(
		menuItem.tagName === 'FORM' ? <SquareCircleIcon /> : <TrashIcon />,
	);
	button.classList =
		'timeline-comment-action color-fg-muted btn-link rgh-actions-run-removal p-1';
	...
	rightControlsContainer.classList.add(
		'd-flex',
		'flex-column-reverse',
		'mt-n2',
		'mb-n2',
...
function init(signal: AbortSignal): void {
	observe(
		'#partial-actions-workflow-runs .Box-row details .octicon-kebab-horizontal',
		addQuickButtons,
		{signal},
	);
}
