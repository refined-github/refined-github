...
function runShortcuts(event: KeyboardEvent): void {
	...
	]).filter(element => (element.classList.contains('js-minimizable-comment-group') ? !isCommentGroupMinimized(element) : true));
	...
	const chosenCommentIndex = Math.min(Math.max(0, currentIndex + direction), items.length - 1);
