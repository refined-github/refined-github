...
import {knownBots} from '../github-helpers/selectors.js';

const knownBotSelector = knownBots.map(bot => `a[href^="/${bot}"]`);
const botLabelTexts = new Set(['AI', 'Bot']);

export function isBotComment(item: Element): boolean {
	...
	for (const label of $$('.Label, [class*="Label"], [class*="Pill"], [data-testid*="pill"], [data-testid*="badge"]', commentHeader)) {
