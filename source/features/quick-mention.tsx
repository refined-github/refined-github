...
function mentionUser({delegateTarget: button}: delegate.Event): void {
	const userMention = button.parentElement!.$('img')!.alt;
...
function init(): void {
	...
	for (const avatar of $$(`.TimelineItem-avatar > [data-hovercard-type="user"]:first-child:not([href="/${getUsername()}"]):not(.rgh-quick-mention)`)) {
