...
const canNotEditLabels = onetime(
	(): boolean => !select.exists(".label-select-menu .octicon-gear")
);

async function removeLabelButtonClickHandler(
	event: delegate.Event<MouseEvent, HTMLButtonElement>
): Promise<void> {
	...
	await api.v3(
		`issues/${getConversationNumber()!}/labels/${removeLabelButton.dataset
			.name!}`,
		{
			method: "DELETE",
		}
...
async function init(): Promise<void> {
	...
	observe(
		".js-issue-labels .IssueLabel:not(.rgh-remove-label-faster-already-added)",
		{
	...
	delegate(
		document,
		".rgh-remove-label-faster:not([disabled])",
		"click",
