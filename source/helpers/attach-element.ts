...
export default function attachElement<NewElement extends Element>(
	...
	{className = 'rgh-' + getCallerID(), append, prepend, before, after, forEach, allowMissingAnchor = false}: Attachment<NewElement>,
): NewElement[] {
...
export function attachElements<NewElement extends Element>(anchors: string | string[], {className = 'rgh-' + getCallerID(), ...options}: Attachment<NewElement>): NewElement[] {
	return $$(`:is(${String(anchors)}):not(.${className})`).flatMap(anchor => attachElement(anchor, {...options, className}));
}
