...
async function isConversationLockedViaDom(): Promise<boolean | undefined> {
	...
	return lockToggle
		? lockToggle.textContent === 'Unlock conversation'
		: undefined;
}

async function isConversationLockedViaReactData(): Promise<
	boolean | undefined
> {
	if (!isInitialLoad()) {
		return;
	...
	return data
		? JSON.parse(data.textContent).payload?.preloadedQueries?.[0].result.data
				.repository?.issue?.locked
		: undefined;
}

export default async function isConversationLocked(): Promise<
	boolean | undefined
> {
