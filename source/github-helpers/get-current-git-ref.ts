...
const typesWithGitReference = new Set(['tree', 'blob', 'blame', 'edit', 'commit', 'commits', 'compare']);
const titleWithGitReference = / at (?<branch>[.\w-/]+)( · [\w-]+\/[\w-]+)?$/i;
...
export default function getCurrentGitReference(): string | undefined {
	...
	const referenceViaPicker = picker && extractCurrentBranchFromBranchPicker(picker);
	if (referenceViaPicker) {
		return referenceViaPicker;
	}
