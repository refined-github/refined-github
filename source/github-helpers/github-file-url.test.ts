...
test('branch', () => {
	const url = new GitHubFileUrl(
		'https://github.com/microsoft/TypeScript/tree/master',
	);
...
test('object', () => {
	const url = new GitHubFileUrl(
		'https://github.com/microsoft/TypeScript/tree/master/src',
	);
	assert.equal(url.user, 'microsoft');
	assert.equal(url.repository, 'TypeScript');
	assert.equal(url.route, 'tree');
...
test('nested object', () => {
	const url = new GitHubFileUrl(
		'https://github.com/microsoft/TypeScript/tree/master/src/index.js',
	);
	assert.equal(url.user, 'microsoft');
	assert.equal(url.repository, 'TypeScript');
	assert.equal(url.route, 'tree');
...
test('change branch', () => {
	...
	assert.equal(
		url.href,
		'https://github.com/microsoft/TypeScript/tree/dev/src/index.js',
	);
	assert.equal(
		String(url),
		'https://github.com/microsoft/TypeScript/tree/dev/src/index.js',
...
test('change filePath', () => {
	...
	assert.equal(
		url.href,
		'https://github.com/microsoft/TypeScript/tree/master/package.json',
	);
	assert.equal(
		String(url),
		'https://github.com/microsoft/TypeScript/tree/master/package.json',
...
test('get filePath from search', () => {
	...
	assert.equal(
		url.search,
		'?after=f23b687b3b89aa95a76193722cdfeff740646670+34',
	);
	assert.equal(
		String(url),
		'https://github.com/yakov116/refined-github/commits/f23b687b3b89aa95a76193722cdfeff740646670/source/features/release-download-count.tsx?after=f23b687b3b89aa95a76193722cdfeff740646670+34',
