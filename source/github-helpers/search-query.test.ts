...
test('getQueryParts with spaces support', () => {
	...
	assert.deepEqual(query.getQueryParts(), [
		'please',
...
test('complex queries with multiple conditions', () => {
	const query = SearchQuery.from({
		q: 'is:issue is:open label:bug author:user milestone:"Q1 2023"',
	});
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
		'is:open',
...
test('queries with special characters in quoted values', () => {
	const query = SearchQuery.from({
		q: 'label:"bug: critical!" milestone:"version 1.0-beta"',
	});
	assert.deepEqual(query.getQueryParts(), [
		'label:"bug: critical!"',
		'milestone:"version 1.0-beta"',
	]);
});

test('queries with colons in quoted values', () => {
	const query = SearchQuery.from({
		q: 'label:"feature: enhancement" comment:"fixes: #1234"',
	});
	assert.deepEqual(query.getQueryParts(), [
...
test('complex parenthesized expressions', () => {
	const query = SearchQuery.from({
		q: 'repo:user/repo (is:issue OR is:pr) label:bug',
	});
	assert.deepEqual(query.getQueryParts(), [
		'repo:user/repo',
		'(is:issue OR is:pr)',
...
test('keys with special characters', () => {
	...
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
		'assignee-review-requested:@me',
	]);
});

test('mixed key-value types in one query', () => {
	const query = SearchQuery.from({
		q: 'is:issue "exact match" label:"needs help" (author:user1 OR author:user2)',
	});
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
		'"exact match"',
...
test('queries with date ranges', () => {
	const query = SearchQuery.from({
		q: 'is:issue created:2023-01-01..2023-12-31',
	});
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
...
test('queries with comparison operators', () => {
	const query = SearchQuery.from({
		q: 'is:issue comments:>10 created:>=2023-01-01',
	});
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
		'comments:>10',
...
test('queries with multiple key-value pairs having the same key', () => {
	const query = SearchQuery.from({
		q: 'is:issue label:bug label:enhancement label:"good first issue"',
	});
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
		'label:bug',
...
test('queries with complex boolean combinations', () => {
	const query = SearchQuery.from({
		q: 'is:issue (label:bug AND author:user) OR (label:feature AND milestone:v1.0)',
	});
	assert.deepEqual(query.getQueryParts(), [
		'is:issue',
		'(label:bug AND author:user)',
		'OR',
		'(label:feature AND milestone:v1.0)',
	]);
});

test('queries with numbers and other special characters in search terms', () => {
	const query = SearchQuery.from({
		q: 'issue#123 PR#456 @user branch:fix/bug-123',
	});
	assert.deepEqual(query.getQueryParts(), [
		'issue#123',
		'PR#456',
...
test('queries with dots in key values', () => {
	const query = SearchQuery.from({
		q: 'filename:test.js extension:.tsx repo:user/repo.js',
	});
	assert.deepEqual(query.getQueryParts(), [
		'filename:test.js',
		'extension:.tsx',
...
test('queries with multiple parenthesized groups', () => {
	const query = SearchQuery.from({
		q: '(is:issue) (is:open) (label:bug) (author:user)',
	});
	assert.deepEqual(query.getQueryParts(), [
		'(is:issue)',
		'(is:open)',
...
test('queries with multiple quoted strings', () => {
	const query = SearchQuery.from({
		q: '"first string" "second string" "third string"',
	});
	assert.deepEqual(query.getQueryParts(), [
		'"first string"',
		'"second string"',
