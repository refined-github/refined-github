...
function validateTsx(file: FeatureFile): void {
	...
	if (
		...
		assert(
			/await expectToken|requiresToken:\s*true|hasToken/.test(String(file.contents())),
			`${file.id} uses the v4 API, so it should include \`await expectToken()\` in its init function, or \`requiresToken: true\`, or if the token is optional, \`hasToken\` anywhere`,
		);
	}
