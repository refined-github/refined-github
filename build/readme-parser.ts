...
const simpleFeatureRegex =
	/^- \[\]\(# "(?<simpleId>[^"]+)"\)(?: 🔥)? (?<simpleDescription>.+)$/gm;
const highlightedFeatureRegex =
	/<p><a title="(?<highlightedId>[^"]+)"><\/a> (?<highlightedDescripion>.+)\n\t+<p><img src="(?<highlightedImage>.+?)">/g;
const featureRegex = regexJoinWithSeparator('|', [
	simpleFeatureRegex,
	highlightedFeatureRegex,
]);
...
function extractDataFromMatch(match: RegExpMatchArray): FeatureMeta {
	...
	const linkLessMarkdownDescription = simpleDescription.replaceAll(
		/\[(.+?)\]\((.+?)\)/g,
		urlExtracter,
	);
	return {
...
export function getFeaturesMeta(): FeatureMeta[] {
	...
	return [...readmeContent.matchAll(featureRegex)]
		...
		.toSorted((firstFeature, secondFeature) =>
			firstFeature.id.localeCompare(secondFeature.id),
		);
}
